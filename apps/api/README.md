...
- **Protection (5.3).** `/api/v1/pets` sits behind `requireAuth` (`src/middleware/require-auth.ts`): no valid session → `401` problem+json, valid session → `req.user`. Every pets query is owner-scoped in the repository, so another user's pet answers the same `404` as a pet that doesn't exist — object-level authorization by construction, recorded in ADR-004.
